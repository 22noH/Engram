...
Engram 위키를 Claude Code·Codex 등 MCP 클라이언트에서 도구로 쓸 수 있습니다 — Engram 앱 없이도. 같은 지식 코어가 돕니다: 내 위키 페이지, 그리고 그것을 지키는 제안·승인 흐름.
...
이후 아무 프로젝트에서: `/engram:wiki-search <검색어>` · `/engram:wiki-save` · `/engram:wiki-list` · `/engram:proposals` · `/engram:approve <id>`.
...
| 도구 | 하는 일 |
|---|---|
| `wiki_search` | 관련 페이지 찾기 |
| `wiki_read` | 페이지 하나 읽기 |
| `wiki_list` | 발행된 페이지 목록 |
| `wiki_propose` | 지식 저장 제안 — 승인해야 저장됨 |
| `list_proposals` | 승인 대기 중인 제안 보기 |
| `approve_proposal` | 대기 중인 제안을 승인해 위키에 반영 |
| `reject_proposal` | 대기 중인 제안 버리기 |
| `wiki_write` | 제안 없이 바로 쓰기 — `--write-mode`일 때만 |
| `ask_brain` | 등록해 둔 다른 모델에 하위 작업 위임 — Engram 앱이 켜져 있을 때만 |

MCP 프롬프트를 지원하는 클라이언트라면 같은 명령(`wiki-search`·`wiki-save`·`proposals`·`approve`)이 거기에도 뜹니다. 플러그인은 편의일 뿐 필수가 아닙니다.

**승인 없이는 아무것도 저장되지 않습니다.** AI가 무언가를 저장하려 하면 쓰던 클라이언트에 확인 창이 바로 뜹니다 — 제목, 어디에 들어가는지, 본문 미리보기와 함께 **Save**/**Cancel**. 취소하면 저장도 대기열 등록도 없습니다. Claude Code 2.1.76+와 Codex v0.119+에서 이렇게 동작하고, 창을 띄울 수 없는 클라이언트에서는 제안이 대기열에 쌓여 채팅에서("제안 보여줘" → "1번 승인") 또는 Engram 앱에서 승인합니다. 어느 쪽이든 사람이 좋다고 해야 반영됩니다. 신뢰하는 자동화가 제안 단계 없이 바로 쓰게 하려면 `--write-mode`를 켜세요.

**검색은 앱이 켜져 있을 때 가장 좋습니다.** Engram 앱이 떠 있으면 MCP가 앱에 넘겨 의미로 검색합니다. 앱 없이 단독으로 돌 때는 임베딩 인덱스를 열지 않아 페이지 본문의 텍스트 일치로만 찾으므로, 말을 바꿔 물으면 덜 걸립니다. 읽기·목록·제안·승인은 어느 쪽이든 동일합니다.

**데이터는 앱과 공유됩니다.** 단독 모드도 데스크톱 앱과 같은 데이터 폴더를 쓰므로, 여기서 먼저 시작하고 나중에 앱을 깔아도 위키가 그대로 이어집니다. 앱이 이미 켜져 있으면 MCP가 자동으로 앱에 브리지합니다(같은 데이터를 두고 다투지 않게).
