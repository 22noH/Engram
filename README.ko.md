...
로드맵 **Phase 8(자체 하네스+MCP)까지 전부 완료** — 데스크톱 앱, 팀 계정, 사람-승인 지식 흐름의 공유 위키, 그리고 양방향 MCP(엔그램이 MCP 클라이언트이자 서버)까지. 앱 없이 쓰는 헤드리스 패키지도 배포됨: [`engram-wiki-mcp`](https://www.npmjs.com/package/engram-wiki-mcp).
...
13. **Phase 8c-1** — MCP 클라이언트: 엔그램 하네스 두뇌가 사용자 등록 MCP 서버의 도구를 사용(공식 SDK·Claude Code와 호환되는 `mcp.json` 포맷·설정창 원클릭 추천 서버)
14. **Phase 8c-2** — MCP 서버: 엔그램이 위키·두뇌 위임을 외부 MCP 클라이언트(Claude Code 등)에 노출 — HTTP `/mcp`(루프백)+stdio 브리지+슬래시 명령 프롬프트
15. **헤드리스 MCP** — `npx engram-wiki-mcp`로 앱 없이 지식 코어 실행: 승인함이 채팅으로, `--write-mode` opt-in, 데이터는 앱과 공유, 앱 실행 중이면 자동 브리지
...
### Claude Code 플러그인 (권장 — 짧은 명령 포함)

설치 한 번에 MCP 연결 **+** 짧은 슬래시 명령까지:
...
이후 어느 프로젝트에서든: `/engram:wiki-search <검색어>` · `/engram:wiki-save` · `/engram:proposals` · `/engram:approve <id>`. 내장 MCP 서버는 `npx -y engram-wiki-mcp`로 돌아간다(앱 불필요, 앱이 켜져 있으면 자동 브리지).

### Claude Code에서 Engram 쓰기 (MCP)

Engram은 위키와 두뇌 위임을 MCP 도구로 노출한다. 같은 PC의 Claude Code·Codex 등 AI 도구에서 접속:
...
**stdio 브리지 (구형 클라이언트용):**
```bash
claude mcp add engram -- node <앱경로>/dist/src/mcp-bridge.js
```
...
| 도구 | 설명 |
|------|------|
| `wiki_search` | 위키 의미검색 (임베딩 기반) |
| `wiki_read` | slug로 위키 페이지 읽기 |
| `wiki_list` | 게시된 위키 페이지 목록 |
| `wiki_propose` | 지식 제안 — 반영은 앱 승인함에서 사람이 승인 |
| `ask_brain` | 등록된 다른 두뇌에 하위작업 위임 |
...
**앱이 이미 켜져 있으면 자동으로 브리지된다.** 헤드리스가 뜰 때 앱의 채팅 포트(기본 47800)가 응답하면, 위키 코어를 새로 열지 않고(같은 LanceDB에 동시 접근하는 위험 회피) 앱의 기존 `/mcp`로 자동 전환한다 — 이때 승인 도구·write 모드는 앱 설정을 그대로 따른다. 앱이 꺼져 있을 때만 헤드리스가 코어를 직접 연다. 반대로 **헤드리스가 코어를 연 채로 앱을 새로 켜는 건 권장하지 않는다** — 짧은 겹침이 생길 수 있으니 앱을 켜기 전에 헤드리스 세션을 먼저 닫는 게 안전하다.
...
Engram은 [GNU AGPL-3.0](LICENSE) 라이선스를 따릅니다. 자유롭게 사용·수정·자가 호스팅할 수 있습니다. **앱이나 `engram-wiki-mcp` CLI를 로컬에서 실행하기만 하는 것에는 아무 의무가 없습니다** — 수정한 버전을 배포하거나 Engram을 네트워크 서비스로 남에게 제공할 때만 같은 라이선스로 소스를 공개하면 됩니다.
