...
- Produces: `private policy(): ChannelPolicy`(lazy 캐시, 테스트 override 지점) · `private channelGate(cap, channelId, post): Promise<boolean>`. 6c-1 플랜은 이 파일의 observe에서 policy를 직접 안 씀(bridge가 함) — 게이트 구조 변경 없음.
...
```ts
  ...
  // (이름 channelGate: 기존 VerificationGate 필드 `gate`와 충돌 회피)
