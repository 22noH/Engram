...
- Produces: `ReviewerAgent.review(criteria, landedSummary)` → `Promise<{ approved: boolean; extraTickets: Array<{ area: string; instruction: string }> }>`. JSON 파싱은 Phase 2 `parseJsonBlock` 패턴 재사용(없으면 보수적으로 approved=false 처리하지 말고 빈 추가티켓 + approved=true 폴백? — 아래 결정).
...
export class ReviewerAgent {
  ...
  async review(criteria: string[], landedSummary: string): Promise<ReviewResult> {
    ...
    return this.parse(r.text);
  }

  // 텍스트에서 첫 균형 JSON 블록 추출(Phase 2 패턴). 실패 시 보수적 폴백.
  private parse(text: string): ReviewResult {
    const start = text.indexOf('{');
    if (start < 0) return { approved: false, extraTickets: [] };
    ...
    for (let i = start; i < text.length; i++) {
      ...
      if (ch === '"') inStr = true;
        ...
        } catch { return { approved: false, extraTickets: [] }; }
      }
    }
    return { approved: false, extraTickets: [] };
  }
}
```
    ...
    for (let i = s; i < text.length; i++) {
      ...
      if (ch === '"') inStr = true;
        ...
        try {
          ...
          return Array.isArray(o.tickets)
...
```ts
  ...
  private parseProposal(text: string): { acceptanceCriteria: string[]; gate: GateCommands } {
    const empty = { acceptanceCriteria: [] as string[], gate: { test: '', build: '', typecheck: '' } };
    const s = text.indexOf('{'); if (s < 0) return empty;
    let depth = 0, inStr = false, esc = false;
    for (let i = s; i < text.length; i++) {
      const ch = text[i];
      if (inStr) { if (esc) esc = false; else if (ch === '\\') esc = true; else if (ch === '"') inStr = false; continue; }
...
```
import 추가: `GateCommands`(project-store), `PermissionFence`.
