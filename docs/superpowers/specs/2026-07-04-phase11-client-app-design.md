...
### 구현 분할(확정) — 플랜 2개
Phase 11이 커서(프론트 전면 이전 + 3영역 + 버튼) **두 플랜으로 쪼갠다**:
- **11a — 앱 토대 + 기능 이전**: `renderer/` React+Vite+TS 부트, Electron `loadFile`
  연결, 타입 ws 프로토콜(`protocol.ts`), 두뇌 http 헬스+ws만(chat.html 서빙 중단),
  §2.5 현 기능 전부 이전, Electron 연동(§2.6), 빌드·패키징. **회귀 0**이 목표.
- **11b — 3영역 + 클릭 버튼**: 영역 3개 네비(채팅 flag-off/Ask/코드) 재편, `actions`
  필드 + `ActionButtons`(승인 confirm), 서버 결정지점 actions 첨부.
- 순서: 11a 먼저(토대), 11b가 그 위에.

### 폴더 배치(확정)
`renderer/`는 **같은 레포 안 최상위**(모노레포식) — 별 레포로 안 뺌. 자체 package.json·
Vite를 갖되 Engram 레포에 함께 커밋. `src/`(nest 빌드 대상) 밖이라 빌드 안 섞임.
