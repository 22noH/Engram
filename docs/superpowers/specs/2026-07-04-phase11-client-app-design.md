...
### 포함
- `web/` 새 폴더 = React+Vite+TS 독립 앱(자체 package.json).
...
- **타입 ws 프로토콜**: 프레임을 한 곳에서 타입 정의 → 서버·`web/` 공유.
- **현 기능 전부 이전**(2.5 인벤토리).
...
|---|---|---|
| 페이지(HTML/JS) 전달 | 두뇌 http가 `chat.html` 서빙, Electron이 `loadURL(http://127.0.0.1:47800)` | 클라가 페이지를 **소유**. Electron `loadFile(web/dist/index.html)` |
...
- **클라 렌더러**: `chatWin`이 `web/dist/index.html`을 `loadFile`로 로드. 렌더러 JS가
  ws로 두뇌에 붙는다.
...
```
web/                     ← 독립 React+Vite+TS 앱(자체 package.json)
  package.json
  vite.config.ts
...
## 2.5 현 기능 이전 인벤토리 (chat.html → web/)
...
- electron-builder `files`: `src/desktop/chat.html` 제거 → `web/dist/**` 추가.
...
- **`web/`(React)**: Vitest + Testing Library. 순수/렌더 위주 —
...
- 신규: `web/**`, `protocol.ts`(공유), electron-builder `files` 갱신, dev/build 스크립트
  (`web:dev`, `web:build`, `desktop:*`가 `web:build` 선행).
- 변경: `SelfMessenger`(http 헬스만), `main.ts`(loadFile + 프로브 진입), preload 경로.
- 삭제: `src/desktop/chat.html`(기능 `web/`로 이전 완료 후).
