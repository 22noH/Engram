...
- 시작 시 `http://127.0.0.1:<port>/` 헬스 프로브(짧은 타임아웃).
  - **상주가 떠 있으면**: LanceDB 동시 접근 위험 → 코어를 직접 열지 않고 **기존
    ...
    (주의: 브리지 모드에선 승인 도구·wiki_write가 앱 /mcp에 없으므로 도구 셋이
    줄어든다 — stderr 안내에 명시. 앱 /mcp 승인 도구 주입은 후속.)
...
- 헤드리스 어댑터가 ProposalStore+ProposalApplier(승인 반영 기존 경로)를 주입.
  ★동시승인 가드: 15a의 in-flight 가드가 ws 경로에 있으므로 헤드리스 어댑터에도
  동일한 in-flight Set(단일 프로세스 내) 적용.

### 3.4 패키징 (publish 준비 완료 상태)
...
- npm publish(계정·이름 — 사용자)·앱 /mcp에 승인 도구 주입(후속)·상호 프로세스
  락파일·원격 접속·헤드리스에서 두뇌(ask_brain)·위키 원격 동기화(15b — 앱 전용 유지).
