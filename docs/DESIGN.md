...
- **Phase 8 — ✦Engram 하네스 (자체 에이전트 하네스)** — **8a·8d·8b-1·8b-2 ✅ 완료 / 8c 남음**. 8a(스펙 `specs/2026-07-13-phase8a-engram-harness-design.md`): API 직접 호출 provider 2종(`anthropic-api` Messages API·`openai-api` OpenAI호환 = Ollama가 claude CLI 없이 동작) + 자체 웹검색(DDG 기본/Brave/Tavily)/웹fetch(SSRF 가드) 미니 도구루프(상한 8) — 단발호출 전 경로 커버, `opts.cwd`(코딩)는 8b까지 명시적 거부. **8d**(스펙 `specs/2026-07-16-phase8d-conductor-brain-design.md`) ✅: 지휘자 두뇌 — 엔그램 하네스 두뇌(anthropic-api·openai-api)에 `ask_brain` 위임 도구를 더해, 대화 중 등록된 다른 두뇌에게 하위작업을 넘긴다(사용자 지목 or 막히면 자율 폴백). 깊이 1단(일꾼은 delegate 없이 호출=재위임 불가)·never-throw·위임 대상은 항상 프로필로 새 인스턴스(고유 세마포어=지휘자와 자원 분리)·`BrainProvider.canDelegate`로 CLI 기본이면 지휘자 오프(회귀0). CLI를 지휘자로 쓰는 건(claude CLI가 codex 호출 등) 8c/MCP. **8b-1**(스펙 `specs/2026-07-17-phase8b1-coding-tool-loop-design.md`) ✅: 엔그램 하네스 두뇌가 `opts.cwd`를 받으면 거부하던 걸 **파일 도구루프**(Read/Write/Edit/Glob/Grep, `coding-tools.ts`)로 교체 = 엔그램이 직접 코딩. 쓰기는 주입 `codeGuard`(=`PermissionFence.assertCodingWrite`=백스톱+프로젝트 스코프)·읽기·쓰기 모두 realpath로 cwd 안 제한(심링크/정션 우회 봉쇄)·셸 없음·가드 없으면 코딩 거부. 하네스는 provider로 사용자 선택, 배포판 기본값(claude-cli) 무변경(선택지 활성화, 강제 아님). **8b-2**(스펙 `specs/2026-07-17-phase8b2-sandbox-shell-design.md`) ✅: 명령 실행(`Bash`) 도구 — 코딩 중 두뇌가 셸 명령을 직접 실행(`shell-tool.ts`, `shell:true`). ★**기본 auto = 아무 명령이나 실행**(Claude Code 자동모드 parity, 목록 관리 없음); 제한은 opt-in `commandMode`(auto/allowlist/off, permissions.json). 안전은 "명령 제한"이 아니라 **사고 방지** = 타임아웃 프로세스트리 강제종료(win `taskkill /T /F`·POSIX 프로세스그룹)+출력 상한+never-throw, **되돌림 주안전망은 이미 있는 `CodingGit` 격리 브랜치**. 명령 판정은 주입(`opts.cmdGuard`=`fence.assertCommandAllowed`, auto면 no-op; `off`면 미주입→Bash 미노출). 설정창에 코딩 명령모드 토글+기본 두뇌(하네스) 드롭다운 추가. OS 샌드박스(AppContainer/컨테이너)·네트워크 하드차단은 미도입(untrusted 코드 격리가 필요해지면 후속). 남은 것: **8c**=MCP 클라이언트(CLI 두뇌를 지휘자로 쓰는 것 포함).
  - **✦설정 전면 UI화 (별도 페이즈, 미착수)**: 지금 설정 상당수가 JSON 파일 편집 전용(권한 상세·coderepos·schedules·wiki-remote·두뇌 세부). 설정창을 config별 섹션으로 전면 개편해 **모든 설정을 앱에서** 만지게 한다. 8b-2에서 자기 설정(셸 모드)+기본 두뇌 드롭다운만 우선 노출했고, 나머지는 이 페이즈로 미룸(착수 시 brainstorming부터). **핵심 통찰** — 지금은 모델뿐 아니라 **하네스(에이전트 도구 루프)까지 외부 CLI(`claude`/`gemini`/`codex`)에서 빌려 쓴다.** "로컬 LLM 사용"조차 [brain.factory.ts](src/brain/brain.factory.ts) 주석대로 *`claude` CLI 껍데기 + env로 모델 엔드포인트만 로컬로* 바꾼 것이라, 하네스는 여전히 `claude`다. **모델 추론은 만들지 않고**(상용 API·로컬 LLM·기존 CLI 중 사용자 선택) 그 위의 **하네스를 Engram 것으로** 갖는 단계. §13.1 궁극 목표("Claude Code를 안 쓰게 되는 것")의 실질 완성. 범위:
