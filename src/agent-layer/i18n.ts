...
const MESSAGES: Record<string, Entry> = {
  ...
  codingAlreadyRunning: { en: (path) => `A coding run for ${path} is already in progress — skipped this start (one run per project).`, ko: (path) => `${path} 코딩이 이미 진행 중이라 이번 시작은 건너뜁니다(프로젝트당 실행 1개).` },
  proposalAutoStart: { en: (targetPath, crit, test, build, typecheck) => `📁 Target: ${targetPath}\n📋 Acceptance criteria:\n${crit}\nGate: test=${test}|build=${build}|typecheck=${typecheck}\nScheduled run — starting without approval.`, ko: (targetPath, crit, test, build, typecheck) => `📁 대상: ${targetPath}\n📋 완성조건:\n${crit}\n게이트: test=${test}|build=${build}|typecheck=${typecheck}\n예약 실행이라 승인 없이 바로 시작할게요.` },
  ...
  cancelScheduleLabel: { en: (i) => `Cancel #${i + 1}`, ko: (i) => `${i + 1}번 취소` },
  cancelThisScheduleLabel: { en: () => 'Cancel this schedule', ko: () => '이 예약 취소' },
