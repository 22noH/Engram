...
it('startProposal(escalate)은 완성조건 게시에 승인/취소 actions를 첨부한다', async () => {
  ...
  expect(approve?.actions).toEqual([
    { label: '✅ Approve', send: '승인', confirm: 'Start autonomous coding?' },
    { label: 'Cancel', send: '취소' },
  ]);
  expect(approve?.text).toContain('Looks right? @Engram approve / to cancel @Engram cancel');
});
