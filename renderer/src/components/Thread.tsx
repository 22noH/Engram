...
export function Thread(props: {
  ...
  if (replies.length === 0) return <Message m={anchor} onSend={props.onSend} />;
  if (replies.length === 1) {
    return (<>
      <Message m={anchor} onSend={props.onSend} />
      <div className="msg reply"><Message m={replies[0]} onSend={props.onSend} /></div>
    </>);
  }
  return (<>
    <Message m={anchor} onSend={props.onSend} />
    <details className="thread" open={!props.collapsed} onToggle={(e) => props.onToggle(!(e.target as HTMLDetailsElement).open)}>
      ...
      {replies.map((r) => <Message key={r.id} m={r} onSend={props.onSend} />)}
