...
// 배포 형태 분리(2026-07-19 설계 §2.2) — team 탭은 preset(config.PRESET) 유무로 게이팅된다.
// 아래 team 배선 테스트들은 preset 배포를 전제로 하므로, config를 preset 있음으로 모의한 뒤
// App을 동적 재import한다(connections.test.ts:28·Channels.test.tsx 패턴과 동일).
async function renderPresetApp() {
  vi.resetModules();
  vi.doMock('./config', () => ({
    WS_URL: 'ws://127.0.0.1:47800', LANG: 'en', ko: false,
    PRESET: { name: 'Test Preset', endpoint: 'ws://preset-test' }, TEAM_CHAT: true,
  }));
  const { default: PresetApp } = await import('./App');
  return PresetApp;
}
...
it('team 모드에서 로그인된 사용자가 보내면 authorId 없이 send 프레임이 나간다(서버가 스탬프)', async () => {
  seedTwoConnections();
  const PresetApp = await renderPresetApp();
  render(<PresetApp />);
  const [homeWS] = FakeWS.instances;
  act(() => { homeWS.open(); });
  act(() => {
